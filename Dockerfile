...
# prepared world. We clone in the builder rather than COPY'ing from the host
# so `git rev-parse` resolves a real revision (baked into the build_info
# event the simulator emits).
#
# Pull from bylins/mud (upstream): master migrated CMake -> Meson, and the
# claude/vibrant-raman-695d14 branch lives there up-to-date. The Bylins-Team
# mirror lags (its push needs a token with `workflow` scope).
ARG MUD_REPO=https://github.com/bylins/mud.git
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    build-essential cmake meson ninja-build pkg-config git gettext \
    python3 python3-pip python3-ruamel.yaml \
    libssl-dev libcurl4-gnutls-dev libexpat1-dev libgtest-dev libyaml-cpp-dev \
    zlib1g-dev libfmt-dev nlohmann-json3-dev \
    && rm -rf /var/lib/apt/lists/*

WORKDIR /src
RUN git clone --depth 1 -b "${MUD_REF}" "${MUD_REPO}" mud
WORKDIR /src/mud

# Build mud-sim via Meson (master migrated off CMake). System yaml-cpp/fmt
# (libyaml-cpp-dev / libfmt-dev above) avoid wrap downloads. Release profile,
# no tests. Binary lands at build_meson/mud-sim.
RUN meson setup build_meson \
        -Dbuild_profile=release \
        -Dbuild_tests=false \
        -Dyaml=system \
        -Dfmt=system \
    && ninja -C build_meson mud-sim
...
COPY --from=build /src/mud/build_meson/mud-sim /opt/mud-sim
...
RUN curl -fsSL -o /app/static/htmx.min.js https://unpkg.com/htmx.org@2.0.4/dist/htmx.min.js && \
    ...
    curl -fsSL -o /app/static/chart.umd.js https://unpkg.com/chart.js@4.4.7/dist/chart.umd.js
